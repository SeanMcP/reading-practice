...
})({"dfSdH":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "6e84b9da5fdcf43a";
"use strict";
...
var _cvcWords = require("./cvc-words");
...
${options.map((option)=>`<button data-answer=${option.text === word.text}>${option.emoji}</button>`
    ).join("")}
</div>
`;
}
function getOptions(current) {
    const shuffled = _utils.shuffle(_cvcWords.easy);
    const options = [
        current
    ];
    for(let i = 0; i < shuffled.length; i++){
        const element = shuffled[i];
        if (element.text !== current.text && !previouslySeen.includes(element.text) && !current.tooClose.includes(element.text)) {
            options.push(element);
            if (options.length === 3) break;
        }
    }
    return _utils.shuffle(options);
}
question();
function speak(word) {
